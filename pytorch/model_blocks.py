...
class ArrDown(nn.Module):
    def __init__(self, in_ch, depth):
        ...
        self.conv_out = nn.Sequential(nn.BatchNorm2d(channel * depth), nn.ReLU(inplace=True), DoubleConv(channel * depth, channel * depth))
...
class ArrUp(nn.Module):
    def __init__(self, in_ch, depth):
        ...
        self.conv_out = nn.Sequential(nn.BatchNorm2d(channel * (10 - depth)), nn.ReLU(inplace=True), DoubleConv(channel * (10 - depth), channel * (10 - depth)))
